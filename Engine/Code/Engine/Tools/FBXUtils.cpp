						...
						const std::map< int, FbxNode* >& jointToNodeMapping )
{
	...
	mb->Begin( PRIMITIVE_TYPE_TRIANGLES, false ); //False = no IBO, as FBX has no concept of indices. 
	...
	if ( nullptr != mesh->GetElementVertexColor() )
		mb->SetVertexDataMaskBit( MESH_VERTEX_ATTRIBUTE_POSITION );
...
//		delete[] currentBoneTransforms;
		...
		if ( importedScene->m_animationSequences.size() > 0 )
