...
//--------------------------------------------------------------------------------------------------------------
void MeshBuilder::Begin( PrimitiveType primitiveType, uint32_t usingIndexBuffer )
{
...
void MeshBuilder::BuildTriangle( const Vector3f& topLeft, const Vector3f& bottomLeft, const Vector3f& bottomRight )
{
	...
	Begin( PrimitiveType::PRIMITIVE_TYPE_TRIANGLES, false ); //IBO won't save anything on a triangle.
	{
									 ...
									 float startY, float endY, unsigned int ySectionCount )
{
	...
	//Note v coordinates may need inversion.
	Begin( PrimitiveType::PRIMITIVE_TYPE_TRIANGLES, true );
	{
...
bool MeshBuilder::ReadFromStream( BinaryReader& reader )
{
	...
	DebuggerPrintf( "Reading FileVersion %hhu vs. Current FileVersion %hhu", fileVersion, s_FILE_VERSION );
...
bool MeshBuilder::ReadDrawInstructions( BinaryReader& reader )
{
	...
	{
		didRead = reader.Read<PrimitiveType>( &instruction.m_type );
