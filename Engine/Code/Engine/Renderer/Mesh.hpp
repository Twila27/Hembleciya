...
{
	DrawInstruction()
		: m_type( PrimitiveType::PRIMITIVE_TYPE_TRIANGLES )
		, m_startIndex( 0 )
		, m_count( 0 )
	...
	DrawInstruction( PrimitiveType type, unsigned int startIndex, unsigned int count, uint32_t useIndexBuffer )
		: m_type( type )
...
public:
	...
	Mesh( BufferUsage usage, const VertexDefinition& vertexDefinition, unsigned int numVertices, void* vertexData, unsigned int numIndices, void* indicesData, unsigned int numDrawInstructions, DrawInstruction* drawInstructions ); //With IBO.
