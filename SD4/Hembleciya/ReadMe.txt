Assignment Name: Save/Load, Items, Features, Terrain
Assignment Number: 04 (Final)
Name: Benjamin Gibson
Class: Software Development IV
Instructor: Ken Harward
...
Final Post-class Roadmap
	...
	Personal:
		...
		* ItemBlueprint
			- "spawnLevel", as above.
	//Need to add notes from personal stuff and phone photos of the markerboard.
		- Add inhabitants="npcblueprintnames" in EnvironmentBlueprints to control which types can spawn in it.


//-----------------------------------------------------------------------------
Special Postmortem Note: dream/amalgamate behaviors made EVERYTHING in A4 harder, swapping maps out back and forth meant features and items getting swapped over or stored on another map, and save/load became a nightmare + amalgam's fused entity IDs making entity pointers less straightforward!
...
A01 Known Issues
	1. The Dungeon generators do not always place a reasonable amount of rooms (too many forbid-on-overlap cases even after softening the comparison).
	2. Smaller maps do not resize nor fit to the center of the screen.
	
Build Notes		
	Developer console input has been disabled for this game.
	...
	Data/
		Audio/
		Fonts/
		XML/ -- note this is where FromData maps are currently stored.
			Biomes/
...
How to Play
	...
	In-Game Controls
					...
					Arrow Keys		 Numpad		 Roguelike Standard	
		...
		Down-Left:					   1				B
		Down-Right: 				   3				N
		A: Toggle Features (e.g. Open/Close Doors)
		R: Rest (+1 Health, Costs 2 Turns)
		Comma: Pick Up Item
		Q: Quaff (Drink) Potion (Last Acquired)
		Period: Drop Unequipped Item (Last Acquired)
		S/F6: Save & Exit Game
...
//-----------------------------------------------------------------------------
Background Music
	The music that plays after dying doesn't stop--this is intentional, letting it become a sort of BGM after you've already died once to get you into things. :)
	It may be disabled for the entirety of the program's execution with the X key, after it begins playing.
	Normal sounds will still play after muting.
	
	
//--------------------------------------------------------------------------------------------------------------
A* Path Debug Visualization Color Legend (F1 in-game to toggle visualization, F4/F5 self-tests single/multi-step)
	1. Blue = Open List
	2. Red = Closed List
	3. Green = Path from Start to Last "Active" Node Tested
	4. Yellow = Goal

			
//--------------------------------------------------------------------------------------------------------------
Conventions
	A01. I use "Biome" in place of "Environment", e.g. Data/XML/Biomes/Caves.Biome.xml or FromDataTest.Biome.xml.
		These XML files in turn use "BiomeBlueprint" root XML elements, instead of "EnvironmentBlueprint" elements.
...
Resource Attributions
	...
	Game Theme Inspiration			Lakota Sioux Tribe's "Hembleciya" ("Vision Quest")
