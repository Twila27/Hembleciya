...
//--------------------------------------------------------------------------------------------------------------
STATIC const int FieldOfViewRaycast::RAYCAST_NUM_STEPS = 100;
STATIC RaycastMode FieldOfViewRaycast::s_raycastMode = AMANATIDES_WOO;

//--------------------------------------------------------------------------------------------------------------
STATIC bool FieldOfViewRaycast::LineOfSightTest_StepAndSample( const Vector2f& start, const Vector2f& end, Map* map, bool isPlayer )
{
	//"Step and Sample" Poor Man's 2D Tile-based Raycast Algorithm
	//
	const int NUM_STEPS_INT = FieldOfViewRaycast::RAYCAST_NUM_STEPS;
	...
	for ( int i = 0; i < NUM_STEPS_INT; i++ )
	{
		...
		CellIndex cellIndex = map->GetIndexForPosition( currentPos2i );

		if ( isPlayer )
			map->GetCells().at( cellIndex ).SetSeen();
...
//--------------------------------------------------------------------------------------------------------------
STATIC bool FieldOfViewRaycast::LineOfSightTest_AmanatidesWoo( const Vector2f& start, const Vector2f& end, Map* map, bool isPlayer )
{
