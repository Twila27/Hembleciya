...
void TheApp::Startup( HINSTANCE applicationInstanceHandle )
{
	...
	if ( g_theConsole != nullptr )
		delete g_theConsole; //Overwrite so it becomes the prompt box for the game.
	g_theConsole = new TheConsole( 0.0, g_theApp->GetScreenHeight() - 60., g_theApp->GetScreenWidth(), g_theApp->GetScreenHeight(), false, Rgba( 1.f, 1.f, 1.f, .5f ), false, .2f, .1f );
}
...
//---------------------------------------------------------------------
Vector2f TheApp::GetScreenCenter() const
{
